...
public partial class StorageViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(MChubSizeString))]
    [NotifyPropertyChangedFor(nameof(TotalSizeString))]
    public partial double MChubBytesRaw { get; set; }
    ...
    public string MChubSizeString => MChubBytesRaw.ToHumanReadableSize(1);
    ...
    public ObservableCollection<GameFolderStorageItem> MChubFolders { get; } = [];
    ...
    public async Task RefreshStorageDataAsync()
    {
        ...
        Logger.Info(
            $"[Storage] Refreshing storage usage for {folders.Count} Minecraft folder(s), MChub data at {portalPath}.");

        MChubFolders.Clear();
        AddFolder(MChubFolders, SettingsLanguageManager.Instance.storage_portalData.CurrentValue(), portalPath,
            portalPath);
        ...
        await Task.Run(() =>
        {
            try
            {
                ...
                var externalBedrockBytes = folders.Sum(folder =>
                {
                    ...
                    var name = folder.DetectedLayout.Kind == MinecraftFolderKind.MChubMc
                        ? "bedrock_instances"
                        : "bedrock_versions";
                ...
                Dispatcher.UIThread.Post(() =>
                {
                    ...
                    MChubBytesRaw = portalRootBytes;
                    MChubFolders[0].SizeBytes = portalRootBytes;
                    ...
                    TotalBytesRaw = MChubBytesRaw + gameBytes;
                    ...
                    Logger.Info(
                        $"[Storage] Storage usage refreshed in {stopwatch.Elapsed}: MChub={portalBytes} bytes, game={gameBytes} bytes.");
                });
            }
            catch (Exception ex)
            {
