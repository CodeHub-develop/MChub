...
internal static class ModpackInstallation
{
        ...
        string? iconUrl, ModpackInstallDialogResult selection)
    {
        ...
        var isMChubMc = MinecraftFolderLayout.TryFindMChubMcRoot(folder, out var portalMcRoot);
        var installFolder = isMChubMc ? Path.Combine(portalMcRoot, "meta") : folder;
        var instancesRoot = isMChubMc ? Path.Combine(portalMcRoot, "instances") : null;
        ...
        var temporaryFolder = Path.Combine(Path.GetTempPath(), "MChub", "modpacks", Guid.NewGuid().ToString("N"));
        ...
        catch (OperationCanceledException exception)
        {
            ...
            await DeleteMChubMcTemporaryLoaderAsync(instancesRoot, installFolder, instanceId);
            throw;
        }
        catch (Exception exception)
        {
    ...
    private static Task DeleteMChubMcTemporaryLoaderAsync(string? instancesRoot, string installFolder,
        string instanceId)
    {
        ...
        string folder, string instanceId)
    {
        ...
        try
        {
            ...
            {
                ImportMChubSettings(instancePath);
                step.ReportProgress(1);
                return Task.CompletedTask;
            });
            await RunStepAsync(context, CommonLanguageManager.Instance.minecraft_refreshInstancesStep.CurrentValue(),
        ...
        string archivePath, string? javaPath, string? instancesRoot)
    {
        ...
        {
            if (hasLoader)
                await MoveMChubMcInstanceAsync(context, folder, effectiveLoaderId, id, instancesRoot);
            else
                WriteMChubMcVanillaInstanceAsync(context, instancesRoot, id, id);
        }

        return minecraft;
    }

    private static void WriteMChubMcVanillaInstanceAsync(TaskExecutionContext context, string instancesRoot,
    ...
    private static void ImportMChubSettings(string instancePath)
    {
        var portalFolder = Path.Combine(instancePath, "MChub");
        if (!Directory.Exists(portalFolder))
            return;

        try
        {
            var exportedConfigPath = Path.Combine(portalFolder, MinecraftInstance.PortableMChubConfigFileName);
            if (File.Exists(exportedConfigPath))
            {
                var targetConfigPath = Path.Combine(instancePath, MinecraftInstance.PortableMChubConfigFileName);
                ...
                if (!string.IsNullOrEmpty(configDirectory))
                {
                    ...
                    Logger.Info($"[Modpack] Imported MChub settings to {targetConfigPath}.");
                }
            }

            var exportedIconPath = Path.Combine(portalFolder, MinecraftInstance.PortableMChubIconFileName);
            if (File.Exists(exportedIconPath))
            {
                var targetIconPath = Path.Combine(instancePath, MinecraftInstance.PortableMChubIconFileName);
                ...
                if (!string.IsNullOrEmpty(iconDirectory))
                {
                    ...
                    Logger.Info($"[Modpack] Imported MChub icon to {targetIconPath}.");
                }
            }
        ...
        {
            Logger.Warning($"[Modpack] Failed to import MChub settings from {portalFolder}: {exception}");
        }
    }

    private static Task MoveMChubMcInstanceAsync(TaskExecutionContext context, string metadataRoot,
