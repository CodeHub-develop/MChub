...
public partial class BedrockInstallation : UserControl
{
    ...
    private async void VersionCard_OnPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        if (folders.Count == 0)
        {
            _viewModel.StatusText = CommonLanguageManager.Instance.minecraft_addMChubFolderFirst.CurrentValue();
            return;
        }
...
public partial class BedrockInstallationViewModel : ObservableObject, IDisposable
{
    ...
    {
        if (IsInstalling ||
            folder.DetectedLayout.Kind is not (MinecraftFolderKind.Standard or MinecraftFolderKind.MChubMc) ||
        ...
        var destination = Path.Combine(folder.FolderPath,
            folder.DetectedLayout.Kind == MinecraftFolderKind.MChubMc ? "bedrock_instances" : "bedrock_versions",
            instanceName);
    ...
    private void UpdateInstallState()
    {
        if (BedrockInstallationService.DefaultInstaller is null)
            ...
            StatusText = CommonLanguageManager.Instance.minecraft_addMChubFolderFirst.CurrentValue();
