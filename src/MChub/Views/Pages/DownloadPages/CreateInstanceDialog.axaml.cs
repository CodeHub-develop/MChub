...
public partial class CreateInstanceDialogViewModel : ObservableObject, IDialogContext, IDisposable
{
    ...
    private static readonly string DefaultIconResource = "MChub.Core.Assets.McIcons.01_grass_block_side.png";
    ...
    private string GetSuggestedIconResource()
    {
        ...
        if (SelectedLoader?.Kind is { } kind)
            return kind switch
            {
                LoaderKind.Fabric => "MChub.Core.Assets.McIcons.05_FabricIcon.png",
                LoaderKind.Forge => "MChub.Core.Assets.McIcons.06_ForgeIcon.png",
                LoaderKind.NeoForge => "MChub.Core.Assets.McIcons.07_NeoForgeIcon.png",
                LoaderKind.Quilt => "MChub.Core.Assets.McIcons.09_QuiltIcon.png",
                _ => DefaultIconResource
            };

        if (IsOptiFineSelected)
            return "MChub.Core.Assets.McIcons.08_OptiFineIcon.png";

        if (SelectedVersion?.Value is VersionManifestEntry vanilla &&
            string.Equals(vanilla.Type, "snapshot", StringComparison.OrdinalIgnoreCase))
            return "MChub.Core.Assets.McIcons.02_crafting_table_front.png";
    ...
    private string? Validate()
    {
        ...
        var instanceDirectory = SelectedMinecraftFolder.DetectedLayout.Kind == MinecraftFolderKind.MChubMc
