...
{
    private const string ResourceName = "MChub.Inject.dll";
    private const string DllName = "MChub.Inject.dll";
    ...
    private static InjectDelegate GetInjector()
    {
        ...
        lock (SyncRoot)
        {
            ...
            var nativePath = Path.Combine(nativeFolder,
                $"MChub.Inject-{Convert.ToHexString(SHA256.HashData(bytes))[..16]}.dll");
