...
namespace MChub.Bedrock;

internal static class BedrockDataIsolation
{
    private const string PreloadDllName = "MChub.Preload.dll";
    private const string PreviousPreloadDllName = "MChub.Preload.Net.dll";
    private const string PreloadResourceName = "MChub.Preload.dll";
    private const string FallbackPreloadDllPrefix = "P";
    private const string LegacyPreloadHookName = "XUserHook.dll";
    private const string BootstrapDllName = "MChub.Bootstrap.dll";
    ...
    {
        var originalPath = Path.Combine(instancePath, "config", "MChub", OriginalExecutableName);
        if (File.Exists(originalPath) && HasMChubImportSection(gameExecutable))
            return;
        Directory.CreateDirectory(Path.GetDirectoryName(originalPath)!);
        File.Copy(gameExecutable, originalPath, true);
    }

    private static bool HasMChubImportSection(string gameExecutable)
    {
        ...
        Action<string, BedrockLogLevel>? log)
    {
        ...
        peFile.Flush();
        log?.Invoke("Repaired duplicate MChub import sections from an interrupted launch preparation.",
    ...
    private static void DeployBootstrapDll(string instancePath)
    {
        ...
        using var stream = assembly.GetManifestResourceStream(BootstrapDllName)
                           ?? throw new InvalidOperationException("MChub.Bedrock.Windows does not include MChub.Bootstrap.dll.");
    ...
    private static void PrepareLaunchInfo(BedrockInstanceConfig config, Action<string, BedrockLogLevel>? log)
    {
        try
        {
            var outputDirectory = Path.Combine(config.InstancePath, "config", "MChub", "launch-info");
            ...
            var launchText = $"©Mojang AB· MChub {version}";
            var languageCount = 0;
            ...
            log?.Invoke($"Prepared MChub launch information for {languageCount} languages.",
                BedrockLogLevel.Information);
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            log?.Invoke($"MChub launch information is unavailable: {exception.Message}", BedrockLogLevel.Warning);
        }
    }
    ...
    {
        var runtimeFolder = Path.Combine(config.InstancePath, "preload", "MChub");
    ...
    {
        var configFolder = Path.Combine(config.InstancePath, "config", "MChub");
