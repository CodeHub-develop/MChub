...
namespace MChub.Bedrock.Preload;
...
internal static unsafe partial class Logger
{
    ...
    public static void Info(string message, string context = "MChub") => Write(LogLevel.Info, message, context);
    public static void Warning(string message, string context = "MChub") => Write(LogLevel.Warning, message, context);
    public static void Error(string message, string context = "MChub") => Write(LogLevel.Error, message, context);
    public static void Success(string message, string context = "MChub") => Write(LogLevel.Success, message, context);

    /// <summary>
    /// 从 <c>UnmanagedCallersOnly</c> 挂钩回调中安全地记录：初始化前直接丢弃（避免在
    /// reverse P/Invoke 上下文中触碰待处理队列），初始化后走原生句柄写入。
    /// </summary>
    internal static void WriteFromHook(LogLevel level, string message, string context = "MChub")
    {
        if (!_ready)
            return;
    ...
    private static string ResolveLogPath(string fileName)
    {
        ...
        string directory = Path.Combine(baseDirectory, "config", "MChub", "logs");
        try
        {
