...
internal static class PathRedirector
{
    ...
    public static string IsolationFolder { get; } = @"config/MChub/isolation";
    ...
    private static void InitializeBaseDirectory(ConfigManager config)
    {
        ...
        BaseDirectory = config.GetConfig("folderPolicyString") switch
        {
            ...
            "portal" => ResolveMChubDirectory(exeDir),
            _ => BaseDirectory,
        };
    ...
    private static string ResolveMChubDirectory(string exeDir)
    {
        string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        return string.IsNullOrEmpty(appData)
            ? Path.Combine(exeDir, IsolationFolder)
            : Path.Combine(appData, "hub.code.MChub", "Bedrock");
    }
