...
{
    private const string XUserHook = "MChub.XUserHook.dll";
    ...
    private static IEnumerable<string> EnumerateCandidates(string directory)
    {
        ...
        {
            Logger.Info("Loading MChub Xbox account hook first");
            yield return xUserHookPath;
        }
        else
        {
            Logger.Info("MChub Xbox account hook is not present; using the default Xbox session");
        }
        ...
        string manifest = Path.Combine(directory, "MChub", "mods.txt");
        ...
        foreach (string line in File.ReadLines(manifest))
        {
            ...
            if (name.Length > 0 && Path.GetFileName(name) == name)
                yield return Path.Combine(directory, "MChub", name);
        }
    }
    ...
    private static void TriggerXUserHook(nint module)
    {
        ...
        if (hookInit == 0)
        {
            Logger.Warning($"MChub.XUserHook.dll 未导出 HookInit；跳过触发");
            return;
        }

        Logger.Info("Calling MChub.XUserHook!HookInit");
