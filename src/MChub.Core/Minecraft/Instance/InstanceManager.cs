...
namespace MChub.Core.Minecraft.Instance;
...
public class InstanceManager
{
    ...
    public List<MinecraftInstance> ScanBedrock(IEnumerable<MinecraftFolderEntry> folders)
    {
        ...
        {
            var layout = folder.DetectedLayout;
            if (layout.Kind is not (MinecraftFolderKind.Standard or MinecraftFolderKind.MChubMc))
                continue;

            var versionsPath = Path.Combine(layout.RootPath,
                layout.Kind == MinecraftFolderKind.MChubMc ? "bedrock_instances" : "bedrock_versions");
