...
internal static class ExternalMinecraftScanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return layout.Kind switch
        {
            ...
            MinecraftFolderKind.MChubMc => await ScanMChubMcAsync(folder, layout, cancellationToken),
            _ => []
        };
    }

    private static async Task<IReadOnlyList<MinecraftInstance>> ScanMChubMcAsync(MinecraftFolderEntry folder,
        MinecraftFolderLayout folderLayout, CancellationToken cancellationToken)
    {
        var result = new List<MinecraftInstance>();
        result.AddRange(await ScanMChubMcJavaAsync(folder, folderLayout, cancellationToken));
        result.AddRange(ScanMChubMcBedrock(folder, folderLayout));
        return result;
    }

    private static async Task<IReadOnlyList<MinecraftInstance>> ScanMChubMcJavaAsync(MinecraftFolderEntry folder,
        MinecraftFolderLayout folderLayout, CancellationToken cancellationToken)
    {
        ...
        foreach (var context in await new MinecraftProvider(new DirectoryInfo(root),
                     [new MChubMcProvider()]).GetMinecraftsAsync(cancellationToken))
        {
            try
            {
                var entry = context.Entry;
                var iconPath = ResolveIcon(entry.InstancePath, "icon.png") ?? ResolveIcon(entry.InstancePath, "Icon.png")
                    ?? ResolveIcon(entry.InstancePath, "MChub.Icon.png");
                result.Add(CreateInstance(context, folder, MinecraftFolderKind.MChubMc, root, metadataRoot,
                    iconPath, entry.Name));
            }
    ...
    private static IReadOnlyList<MinecraftInstance> ScanMChubMcBedrock(MinecraftFolderEntry folder,
