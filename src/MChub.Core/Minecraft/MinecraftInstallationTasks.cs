...
using Data = MChub.Core.Const.Data;
...
public static class MinecraftInstallationTasks
{
    ...
    {
        if (folder.DetectedLayout.Kind == MinecraftFolderKind.MChubMc)
        {
            await RunMChubMcInstallationAsync(context, vanilla, folder, versionId, selectedEntries, javaPath);
            return;
        }
    ...
    private static async Task RunMChubMcInstallationAsync(TaskExecutionContext context, VersionManifestEntry vanilla,
        MinecraftFolderEntry folder, string versionId,
        IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath)
    {
        ...
        try
        {
                ...
                CommonLanguageManager.Instance.minecraft_generatingInstanceConfig.CurrentValue(), step =>
            {
                ...
                {
                    WriteMChubMcMinimalInstanceJson(instanceDirectory, versionId, vanillaId);
                }
    ...
    public static void WriteMChubMcMinimalInstanceJson(string instanceDirectory, string instanceId, string vanillaId)
    {
