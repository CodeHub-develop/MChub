    ...
    string DisplayName)
{
    ...
    public bool SupportsInstallation => Kind == MinecraftFolderKind.MChubMc;
    ...
    private static bool IsMChubMcRoot(string path)
    {
        ...
        return Path.GetFileName(Path.GetFullPath(path))
            .Equals("hub.code.MChub.minecraft", StringComparison.OrdinalIgnoreCase);
    }

    public static bool TryFindMChubMcRoot(string path, out string root)
    {
        var current = new DirectoryInfo(path);
        while (current != null)
        {
            if (IsMChubMcRoot(current.FullName))
            {
    ...
    public static MinecraftFolderLayout Detect(string path)
    {
        ...
        if (IsMChubMcRoot(selected))
            return new MinecraftFolderLayout(MinecraftFolderKind.MChubMc, selected, selected, "MChub MC");
        if (TryFindMChubMcRoot(selected, out var portalMcRoot))
            return new MinecraftFolderLayout(MinecraftFolderKind.MChubMc, portalMcRoot, portalMcRoot, "MChub MC");
    ...
    public static MinecraftFolderLayout FromFolderKind(MinecraftFolderKind kind, string path)
    {
        ...
        var displayName = kind switch
        {
            ...
            MinecraftFolderKind.MChubMc => "MChub MC",
