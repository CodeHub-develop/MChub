...
using MChub.Core.Minecraft.Graphics;
using MChub.Core.Minecraft.Instance;
using MChub.Core.Minecraft.Instance.Bedrock;
...
{
    public const string PortableMChubConfigFileName = "MChub.config.json";

    public const string PortableMChubIconFileName = "MChub.Icon.png";
    ...
    public bool CanModifyVersion =>
        ...
        Layout?.Kind == MinecraftFolderKind.MChubMc;
    ...
    {
        (true, false) => CommonLanguageManager.Instance.minecraft_bedrockScopeMChubInstance.CurrentValue(),
        (false, false) => CommonLanguageManager.Instance.minecraft_bedrockScopeMChub.CurrentValue(),
    ...
    private MinecraftInstanceConfig GetInstanceConfig()
    {
        ...
        if (File.Exists(configPath))
            try
            {
                var loadedConfig = Type == MinecraftInstanceType.Java
                    ? JsonSerializer.Deserialize<JavaInstanceConfig>(File.ReadAllText(configPath), MChubJson.Options)
                    : JsonSerializer.Deserialize<MinecraftInstanceConfig>(File.ReadAllText(configPath), MChubJson.Options);
        ...
        File.WriteAllText(configPath, JsonSerializer.Serialize(config, config.GetType(), MChubJson.Options));
        return config;
    }
    ...
    {
        return Path.Combine(InstanceFolderPath, PortableMChubConfigFileName);
    }
    ...
    public IReadOnlyList<string> GetDeletionPaths()
    {
        ...
        if (layout.Kind == MinecraftFolderKind.MChubMc && MinecraftEntry is { } entry)
        {
    ...
    {
        return Path.Combine(GetSpecialFolder(MinecraftSpecialFolder.InstanceFolder), PortableMChubIconFileName);
    }
    ...
    private static Bitmap LoadBitmapFromAssembly(string fileName, int width)
    {
        ...
        var resourcePath = $"MChub.Core.Assets.McIcons.{fileName}";

        using var stream = assembly.GetManifestResourceStream(resourcePath);
        if (stream == null)
        {
            var defaultPath = "MChub.Core.Assets.McIcons.01_grass_block_side.png";
...
public partial class MinecraftInstanceConfig : ObservableObject
{
    ...
    [ObservableProperty] public partial MChubVisibleMode MChubVisibleMode { get; set; } = MChubVisibleMode.NoOperation;
