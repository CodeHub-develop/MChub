...
public sealed class BlockListService
{
    ...
    private void Save()
    {
        ...
        File.WriteAllText(tempPath, JsonSerializer.Serialize(Document, MChubJson.Options));
    ...
    private BlockListDocument Load()
    {
        try
        {
            return File.Exists(_path)
                ? JsonSerializer.Deserialize<BlockListDocument>(File.ReadAllText(_path), MChubJson.Options) ??
