...
public sealed class FavoriteCollectionService
{
    ...
    public void Save()
    {
        ...
        File.WriteAllText(temporaryPath, JsonSerializer.Serialize(Document, MChubJson.Options));
    ...
    {
        var imported = JsonSerializer.Deserialize<FavoriteCollectionDocument>(File.ReadAllText(path), MChubJson.Options);
    ...
    public void Export(FavoriteCollection collection, string path)
    {
        ...
        File.WriteAllText(path, JsonSerializer.Serialize(document, MChubJson.Options));
    }
    ...
    private FavoriteCollectionDocument Load()
    {
        try
        {
            return File.Exists(_path)
                ? JsonSerializer.Deserialize<FavoriteCollectionDocument>(File.ReadAllText(_path), MChubJson.Options) ??
