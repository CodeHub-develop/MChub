...
{
    private const string BackupFolderName = ".MChubModifyBackup";
        ...
        VersionManifestEntry vanilla, IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath)
    {
        ...
        if (!instance.CanModifyVersion)
            throw new InvalidOperationException(CommonLanguageManager.Instance.versionModify_onlyMChub.CurrentValue());
        ...
        var isMChubMc = MinecraftFolderLayout.TryFindMChubMcRoot(folderPath, out var portalMcRoot);
        var metadataRoot = isMChubMc ? Path.Combine(portalMcRoot, "meta") : folderPath;
        var instanceDirectory = isMChubMc
            ? instance.Layout?.InstanceRoot ?? Path.Combine(portalMcRoot, "instances", instanceId)
            : Path.Combine(folderPath, "versions", instanceId);
        var hasLoaders = selectedEntries.Count > 0;
        var usesTempBase = !isMChubMc && hasLoaders &&
        ...
        try
        {
            ...
            {
                if (isMChubMc)
                    await InstallLoadersMChubMcAsync(context, metadataRoot, instanceDirectory, instanceId, vanilla,
            ...
            {
                if (isMChubMc)
                    await InstallPureVanillaMChubMcAsync(context, metadataRoot, instanceDirectory, instanceId, vanilla,
                        sourceRoots);
                else
    ...
    private static async Task InstallPureVanillaMChubMcAsync(TaskExecutionContext context, string metadataRoot,
        string instanceDirectory, string instanceId, VersionManifestEntry vanilla, IEnumerable<string> sourceRoots)
    {
        await Task.Run(() =>
        {
        ...
        {
            MinecraftInstallationTasks.WriteMChubMcMinimalInstanceJson(instanceDirectory, instanceId, vanilla.Id);
            step.ReportProgress(1);
            return Task.CompletedTask;
        });
    }

    private static async Task InstallLoadersMChubMcAsync(TaskExecutionContext context, string metadataRoot,
        string instanceDirectory, string instanceId, VersionManifestEntry vanilla,
        IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath,
        IEnumerable<string> sourceRoots)
    {
