...
internal static class CacheDatabase
{
    ...
    private static string DatabasePath => Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "hub.code.MChub", "Cache", "cache.db");
    ...
    private static void MigrateLegacyNews(SqliteConnection connection)
    {
        ...
        using (var selectCommand = connection.CreateCommand())
        {
            ...
            while (reader.Read())
            {
                ...
                try
                {
                    response = JsonSerializer.Deserialize<PatchNotesResponse>(reader.GetString(1), MChubJson.Options);
                }
