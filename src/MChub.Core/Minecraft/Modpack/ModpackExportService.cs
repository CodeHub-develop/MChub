...
namespace MChub.Core.Minecraft.Modpack;
...
public sealed record ModpackExportOptions
{
    ...
    public bool IncludeMChubSettings { get; init; } = true;
    public bool IncludeMChubIcon { get; init; } = true;
}
...
public sealed class ModpackExportService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheFolder = Path.Combine(Path.GetTempPath(), "MChub", "ModpackExport",
        ...
        try
        {
            ...
            if (options.IncludeMChubSettings)
                CopyMChubSettings(instance, overridesFolder);

            if (options.IncludeMChubIcon)
                CopyMChubIcon(instance, overridesFolder);
    ...
    private static void CopyMChubSettings(MinecraftInstance instance, string overridesFolder)
    {
        ...
        try
        {
            var targetDirectory = Path.Combine(overridesFolder, "MChub");
            Directory.CreateDirectory(targetDirectory);
            File.Copy(configPath, Path.Combine(targetDirectory, MinecraftInstance.PortableMChubConfigFileName), true);
        }
        catch (Exception exception)
        {
            Logger.Warning(string.Format(LogLanguageManager.Instance.modpack_copyMChubSettingsFailed.CurrentValue(), exception));
        }
    }

    private static void CopyMChubIcon(MinecraftInstance instance, string overridesFolder)
    {
        ...
        try
        {
            var targetDirectory = Path.Combine(overridesFolder, "MChub");
            Directory.CreateDirectory(targetDirectory);
            File.Copy(iconPath, Path.Combine(targetDirectory, MinecraftInstance.PortableMChubIconFileName), true);
        }
        catch (Exception exception)
        {
            Logger.Warning(string.Format(LogLanguageManager.Instance.modpack_copyMChubIconFailed.CurrentValue(), exception));
        }
    }
