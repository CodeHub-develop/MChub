...
namespace MChub.Core.Module.Update;
...
public static class UpdateApp
{
    ...
    internal static UpdateAsset SelectAsset(UpdateRelease release, string packageType)
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            {
                "installer" => "MChub.win.x64.installer.zip",
                "portable" => "MChub.win.x64.portable.zip",
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            expectedName = packageType switch
            {
                "appimage" => "MChub.linux.x64.AppImage",
                "deb" => "MChub.linux.x64.deb",
                "rpm" => "MChub.linux.x64.rpm",
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            expectedName = $"MChub.osx.mac.{arch}.app.zip";
        }
        else
        {
    ...
    private static ProcessStartInfo PrepareWindowsPortable(string zipPath, string updateDirectory, string target)
    {
        ...
        File.WriteAllText(script, $$"""
                                    ...
                                    try {
                                      ...
                                      if (Get-Process -Id $pidToWait -ErrorAction SilentlyContinue) { throw 'MChub did not exit in time.' }
                                      ...
                                      if ($process.HasExited) { throw 'The updated MChub exited immediately.' }
    ...
    private static PreparedUpdate PrepareAppImage(string packagePath, string updateDirectory)
    {
        ...
        File.WriteAllText(workerScript, $$"""
                                          ...
                                          log={{Sh(log)}}
                                          exec >>"$log" 2>&1
                                          echo "MChub AppImage update started: $(date -Is)"
                                          ...
                                          if [ -n "${PKEXEC_UID:-}" ]; then
                                            ...
                                            if ! command -v runuser >/dev/null 2>&1; then
                                              echo "The system does not provide runuser; MChub was replaced but not restarted."
                                              exit 1
                                            fi
                                            runuser -u "$user" -- env \
                                              HOME="$home" \
                                          ...
                                          sleep 5
                                          rm -rf "$backup"
                                          echo "MChub AppImage update completed: $(date -Is)"
                                          """);
        File.WriteAllText(launcherScript, $$"""
                                              ...
                                              log={{Sh(log)}}
                                              echo "MChub update authentication accepted: $(date -Is)" >>"$log"
        ...
        string packagePath, string updateDirectory, string packageType, string processPath)
    {
        ...
        File.WriteAllText(workerScript, $$"""
                                          ...
                                          log={{Sh(log)}}
                                          exec >>"$log" 2>&1
                                          echo "MChub package update started: $(date -Is)"
                                          pid='{{Environment.ProcessId}}'
                                          ...
                                          echo "Starting the updated MChub process as $user."
                                          if ! command -v runuser >/dev/null 2>&1; then
                                            echo "The system does not provide runuser; MChub was installed but not restarted."
                                            exit 1
                                          fi
                                          runuser -u "$user" -- env \
                                            HOME="$home" \
                                            ...
                                            XDG_RUNTIME_DIR="$xdg_runtime_dir" \
                                            nohup "$target" >/dev/null 2>&1 &
                                          echo "MChub package update completed: $(date -Is)"
                                          """);
        File.WriteAllText(launcherScript, $$"""
                                            #!/bin/sh
                                            set -eu
    ...
    private static ProcessStartInfo PrepareMacApp(string packagePath, string updateDirectory, string processPath)
    {
        ...
        var replacement = Path.Combine(extracted, "MChub.app");
        if (!File.Exists(Path.Combine(replacement, "Contents", "MacOS", "MChub.Desktop")))
