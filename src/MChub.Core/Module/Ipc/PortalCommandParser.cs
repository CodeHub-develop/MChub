...
public enum MChubCliParseStatus
{
    NotACommand,
    Help,
    Error,
...
public static class MChubCommandParser
{
    ...
    public static MChubCliParseStatus Parse(string[] args, out MChubCommand? command, out string? error)
    {
        command = null;
        error = null;
        if (args.Length == 0) return MChubCliParseStatus.NotACommand;

        var first = args[0].Trim();
        if (first is "help" or "--help" or "-h" or "/?" or "-?")
            return MChubCliParseStatus.Help;
        ...
        return first.ToLowerInvariant() switch
        {
            ...
            _ => MChubCliParseStatus.NotACommand
        };
    }
    ...
    private static MChubCliParseStatus ParseInstallCli(string[] args, out MChubCommand? command, out string? error)
    {
        command = null;
        error = null;
        if (args.Length < 2)
        {
            ...
            return MChubCliParseStatus.Error;
        }
        ...
        if (sub == "modpack")
        {
            ...
            if (!TryValidateProvider(options.Provider, out error)) return MChubCliParseStatus.Error;
            command = new MChubCommand
            {
            ...
            return MChubCliParseStatus.Command;
        }
        ...
        command = new MChubCommand
        {
            Kind = options.Loaders.Count > 0 ? MChubCommandKind.DownloadLoader : MChubCommandKind.DownloadVanilla,
    ...
    private static MChubCliParseStatus ParseLaunchCli(string[] args, out MChubCommand? command, out string? error)
    {
        command = null;
        ...
        out string? error)
    {
        ...
        int? serverPort = null;
        var loaders = new List<MChubLoaderSpec>();
        options = null!;
        error = null;
    ...
    private static MChubLoaderSpec ParseLoaderSpec(string value)
    {
        var separator = value.IndexOf('@');
        return separator > 0
            ? new MChubLoaderSpec(value[..separator].Trim(), value[(separator + 1)..].Trim())
            : new MChubLoaderSpec(value.Trim(), null);
    }

    private static MChubCliParseStatus ParseUri(string raw, out MChubCommand? command, out string? error)
    {
        command = null;
        error = null;
            ...
            case "install" or "download":
            {
                ...
                switch (sub)
                {
                    case "vanilla" or "loader":
                    {
                        ...
                        command = new MChubCommand
                        {
                            Kind = loaders.Count > 0
                                ? MChubCommandKind.DownloadLoader
                                : MChubCommandKind.DownloadVanilla,
                            Version = version,
                            Loaders = loaders,
                            Folder = GetValue(parameters, "folder", "dir"),
                    ...
                    case "modpack":
                    {
                        ...
                        if (!TryValidateProvider(provider, out error)) return MChubCliParseStatus.Error;
                        command = new MChubCommand
                        {
                            Kind = MChubCommandKind.DownloadModpack,
                            Source = source,
