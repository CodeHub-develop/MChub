...
/// </summary>
internal static class MChubCliHeadless
{
    ...
    public static bool TryRun(string[] args, out int exitCode)
    {
        ...
        {
            case "--version" or "-v":
                MChubCommandRegistration.RegisterAsync().GetAwaiter().GetResult();
                PrintVersion();
                return true;
            ...
            case "help" or "--help" or "-h" or "/?" or "-?":
                MChubCommandRegistration.RegisterAsync().GetAwaiter().GetResult();
                Write(MChubCommandParser.GetHeadlessUsageText());
                return true;
            default:
                return false;
        }
    ...
    private static void PrintVersion()
    {
        ...
        var title = string.IsNullOrWhiteSpace(version.VersionTitle) ? "MChub" : version.VersionTitle;
        ...
        switch (MChubCommandParser.Parse(args, out var command, out var error))
        {
            case MChubCliParseStatus.Error:
                Write(string.Format(
                    ...
                    Environment.NewLine, Environment.NewLine, MChubCommandParser.GetHeadlessUsageText()));
                return 1;
