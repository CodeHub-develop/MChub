...
internal static class CliHeadlessLauncher
{
    ...
    public static int Run(MChubCommand command)
    {
    ...
    private static async Task<int> LaunchJavaAsync(MinecraftInstance instance, MinecraftEntry entry,
        CliSettings settings, MChubCommand command)
    {
        HttpUtil.Configure(false, null, "MChub/CLI");
        ...
        var mcProcess = await new Iridium.Launch.Launcher(
            new MChubGameArgumentParser(new ArgumentParser(), extraGameArguments))
    ...
    private static MinecraftInstance? ResolveLaunchInstance(MChubCommand command, List<MinecraftFolderEntry> folders)
    {
    ...
    private static CliSettings LoadSettings()
    {
        ...
        var settingsPath = MChub.Core.Const.ConfigPath.SettingDataPath;
        if (!File.Exists(settingsPath))
            return settings;

        try
        {
            var loaded = JsonSerializer.Deserialize<CliSettings>(File.ReadAllText(settingsPath), MChubJson.Options)
                         ?? settings;
    ...
    private static LaunchConfig CreateLaunchConfig(MinecraftInstance instance, Account account, JavaEntry java,
        MinecraftLaunchOptions options, MChubCommand command, Dictionary<string, string> placeholders,
    ...
    {
        MChubCommandService.WriteConsoleLines(lines);
    }
}
