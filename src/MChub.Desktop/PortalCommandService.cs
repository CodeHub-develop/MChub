...
internal static partial class MChubCommandService
{
    private const string PipeName = "hub.code.MChub.Command";
    ...
    public static bool TryHandleStartupArgs(string[] args)
    {
        switch (MChubCommandParser.Parse(args, out var command, out var error))
        {
            ...
            case MChubCliParseStatus.Help:
                WriteConsole(MChubCommandParser.GetHeadlessUsageText());
                return true;
            ...
            case MChubCliParseStatus.Command when command is not null:
                ...
                MChubCommandQueue.Enqueue(command);
                return false;
            default:
                return false;
        }
    ...
    internal static bool TryForwardToRunningInstance(MChubCommand command, int attempts = 1)
    {
    ...
    private static bool TryForwardOnce(MChubCommand command)
    {
        try
        {
        ...
        while (true)
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(json))
                    continue;
                if (JsonSerializer.Deserialize<MChubCommand>(json) is not { } command) continue;
