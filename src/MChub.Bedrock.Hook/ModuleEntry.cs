...
using MChub.Bedrock.Hook.Mods;
using MChub.Bedrock.Hook.Network;
...
/// 与显式导出 HookInit（由 MChub.Preload 在 LoadLibraryW 后调用），
/// 两者均只触发一次，确保工作线程被拉起。
/// </summary>
internal static unsafe class ModuleEntry
{
    ...
    private static void WriteBootMarker(string state)
    {
        try
        {
            ...
            string path = Path.Combine(directory, "config", "MChub", "logs", "hook-boot.log");
    ...
    private static nint GetSelfModuleBase()
    {
        try
        {
            return NativeMethods.GetModuleHandleW("MChub.XUserHook.dll");
        }
        catch
        {
            return 0;
    ...
    private static uint WorkerThread(nint parameter)
    {
        ...
        XUserBridge.Info("MChub.XUserHook worker started");
        XUserBridge.Info($"MChub.XUserHook base=0x{GetSelfModuleBase():X} pid={Environment.ProcessId}");
