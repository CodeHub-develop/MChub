...
Copy-Item "<tmp>\font_*\iconfont.json" "E:\MChub\docs\ai\iconfont.json" -Force
Copy-Item "<tmp>\font_*\iconfont.ttf" "E:\MChub\src\MChub\Assets\Fonts\iconfont.ttf" -Force
```
编辑 `src\MChub\Module\IconResources.cs` 的 `Glyphs` 字典：
...
- 窗口按钮：`src\MChub\Styles\Style.axaml` 的 `tio|TioTitleBar` Setter `Value="&#xXXXX;"`

重建验证：
```powershell
dotnet build "E:\MChub\src\MChub.Desktop\MChub.Desktop.csproj" -c Debug
```

## 渲染图标
XAML：
```xml
xmlns:icons="using:MChub.Module"
...
## TTF 更换
- 嵌入资源（`avares://MChub/Assets/Fonts/iconfont.ttf`）：替换文件 + 重建即可
